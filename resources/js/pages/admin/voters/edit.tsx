...
export default function UserEdit() {
    ...
    const { data, setData, patch, processing, errors } = useForm({ student_id: voter.student_id ?? '', first_name: voter.first_name ?? '', middle_name: voter.middle_name ?? '', last_name: voter.last_name ?? '', gender: voter.gender ?? '', course: voter.course ?? '', has_voted: voter.has_voted ?? false });

    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        patch(route('admin.voters.update', voter.id));
    };
            ...
            <Head title="Edit User" />
                ...
                <form onSubmit={submit} className="space-y-4">
                    <div>
                        ...
                        <button type="submit" disabled={processing} className="rounded bg-blue-600 px-3 py-2 text-white">Save changes</button>
                    </div>
                </form>
        </AppLayout>
    );
